        ...
        <div className="overflow-y-auto px-6 py-4 space-y-3 flex-1">
          ...
          {form.is_fall_risk === 'yes' && (
            <div className="bg-surface-2 border border-rule rounded-lg p-4 space-y-3">
              ...
              <Field label="Is the Bed Alarm on?" value={form.bed_alarm_on} onChange={set('bed_alarm_on')} options={YES_NO} />
              ...
              <Field label="Is the Call Light within reach?" value={form.call_light_reach} onChange={set('call_light_reach')} options={YES_NO} />
              <Field label="Does the patient have a Fall Risk Wristband on?" value={form.fall_wristband} onChange={set('fall_wristband')} options={YES_NO} />
              <Field label="Does the patient have Non-Slip Socks on?" value={form.non_slip_socks} onChange={set('non_slip_socks')} options={YES_NO} />
              <Field label="Is there a Gait Belt in the room?" value={form.gait_belt_present} onChange={set('gait_belt_present')} options={YES_NO} />
              <Field label="Is there a Walker in the room?" value={form.walker_present} onChange={set('walker_present')} options={YES_NO} />
              <Field label="Is there a Posey Alarm in the room?" value={form.posey_alarm_present} onChange={set('posey_alarm_present')} options={YES_NO} />
              ...
              <Field label="Is there a Shower Chair in the bathroom?" value={form.shower_chair_present} onChange={set('shower_chair_present')} options={YES_NO} />
              <Field label="Is there a Bedside Commode in the bathroom?" value={form.bedside_commode_present} onChange={set('bedside_commode_present')} options={YES_NO} />
            </div>
          )}
          ...
          {form.is_hapi_risk === 'yes' && (
            <div className="bg-surface-2 border border-rule rounded-lg p-4 space-y-3">
              ...
              <Field label="Is the patient on a Specialty Bed?" value={form.specialty_bed_yn} onChange={set('specialty_bed_yn')} options={YES_NO} />
              ...
              <Field label="Is the patient turned to their side with Purple Wedges?" value={form.turned_with_wedges} onChange={set('turned_with_wedges')}
                options={[['', 'Select...'], ['yes', 'Yes (turned on their side with the wedges)'], ['no', 'No (laying flat or pillows used instead)'], ['sitting_in_chair', 'No (the patient is sitting in the chair)'], ['not_applicable', 'Not applicable']]} />
              ...
              <InfoBlock>"How to Effectively FLOAT Heels" — heels should be fully off the surface of the bed, not resting on a pillow or cushion alone.</InfoBlock>
              <Field label="Are the patient's heels offloaded properly?" value={form.heels_offloaded} onChange={set('heels_offloaded')}
                options={[['', 'Select...'], ['yes', 'Yes (heels are off the surface of the bed)'], ['no', 'No (heels are touching the surface of the bed)'], ['not_applicable', 'Not applicable']]} />
          ...
          <div className="pt-2 space-y-3">
            ...
            {form.is_hapi_risk === 'yes' && form.patient_refused_education !== 'yes' && (
              <>
            ...
            <Field label="Have you been educated today about pressure injuries by your care team (nurse or PCT)?" value={form.already_educated_today} onChange={set('already_educated_today')} options={YES_NO_UNABLE} />
          </div>
